...
func (btc *BigtableAdapter) DropAllRows(ctx context.Context, data *types.TruncateTableStatementMap) error {
	...
	hasRows, err := btc.hasRows(ctx, data.Keyspace(), data.Table())
	if err != nil {
		return err
	}
	if !hasRows {
...
func (btc *BigtableAdapter) hasRows(ctx context.Context, keyspace types.Keyspace, tableName types.TableName) (bool, error) {
