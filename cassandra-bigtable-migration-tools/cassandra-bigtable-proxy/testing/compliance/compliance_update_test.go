...
func TestUpdateNullValues(t *testing.T) {
	...
	t.Run("Placeholders", func(t *testing.T) {
		...
		// 2. Update to null using placeholders (don't set zip_code to nil to prevent an empty row)
		err = session.Query(`UPDATE bigtabledevinstance.user_info SET code = ?, credited = ?, text_col = ?, is_active = ?, birth_date = ?, balance = ?, tags = ?, extra_info = ?, list_text = ? WHERE name = ? AND age = ?`,
			nil, nil, nil, nil, nil, nil, nil, nil, nil, pkName, pkAge).Exec()
		require.NoError(t, err)
		...
		err = session.Query(`SELECT code, credited, text_col, is_active, birth_date, balance, tags, extra_info, list_text FROM bigtabledevinstance.user_info WHERE name = ? AND age = ?`, pkName, pkAge).
			Scan(&code, &credited, &textCol, &isActive, &retrievedBirthDate, &balance, &retrievedTags, &retrievedExtraInfo, &retrievedListText)
		require.NoError(t, err)
	...
	t.Run("Literals", func(t *testing.T) {
		...
		// 2. Update to null using NULL literals  (NOT nulling zip_code ensures it's not an empty row)
		err = session.Query(`UPDATE bigtabledevinstance.user_info SET code = NULL, credited = NULL, text_col = NULL, is_active = NULL, birth_date = NULL, balance = NULL, tags = NULL, extra_info = NULL, list_text = NULL WHERE name = ? AND age = ?`,
			pkName, pkAge).Exec()
		require.NoError(t, err)
