...
func isHandledSelectStmt(l *lexer, keyspace Identifier) (handled bool, stmt Statement, err error) {
	...
	t := untilToken(l, tkFrom) //CC - loop untill reach till from or end of file

	//CC -  If reached to the end of file raise an error as from token is manadatory
	if tkFrom != t {
		return false, nil, errors.New("expected 'FROM' in select statement")
	}

	// CC- check if table identifier exist after from statement else throw error
	...
	//CC - if keyspance or qualifyingKeyspace is not equal to system then it return isHandled as false
	var isNotASystemSchema = !keyspace.equal("system") && !qualifyingKeyspace.equal("system")
	var isNotASystemSchemaSchema = !keyspace.equal("system_schema") && !qualifyingKeyspace.equal("system_schema")
	var isNotSystemVirtualSchema = !keyspace.equal("system_virtual_schema") && !qualifyingKeyspace.equal("system_virtual_schema")

	if err != nil || (isNotASystemSchema && isNotASystemSchemaSchema && isNotSystemVirtualSchema) || !isSystemTable(table) {
		return false, nil, err
	}
	// CC - capture keyspance name
	var capturedKeySpace string
	if qualifyingKeyspace.id != "" {
		capturedKeySpace = qualifyingKeyspace.id
	} else if keyspace.id != "" {
		capturedKeySpace = keyspace.id
	}

	selectStmt := &SelectStatement{Keyspace: capturedKeySpace, Table: table.id}

	// This only parses the selectors if this is a query handled by the proxy
	// CC - revert lexer to from statement
