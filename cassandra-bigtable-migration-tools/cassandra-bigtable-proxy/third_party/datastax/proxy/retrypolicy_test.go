...
func TestString(t *testing.T) {
	var retryDecision RetryDecision = 0
	retryDecision = 0

	res := retryDecision.String()
	assert.NotEqualf(t, "unknown", res, "should not be equal")
	...
	retryDecision = 404
	res = retryDecision.String()
	assert.Equalf(t, "unknown", res, "should be equal")
}
