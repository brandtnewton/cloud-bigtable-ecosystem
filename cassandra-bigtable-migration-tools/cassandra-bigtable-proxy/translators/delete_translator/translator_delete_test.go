...
func TestTranslator_TranslateDeleteQuerytoBigtable(t *testing.T) {
	...
	tests := []struct {
		...
		fields          fields
		args            args
		want            *translators.PreparedDeleteQuery
		wantErr         bool
		...
		},
		{
			...
			fields: fields{
				SchemaMappingConfig: translators.GetSchemaMappingConfig(types.BigEndianEncoding),
			},
			want: &translators.PreparedDeleteQuery{
				Query:     "DELETE FROM test_keyspace.user_info WHERE name='test' AND age=15",
				QueryType: "DELETE",
