...
public class FlattenArrayElement<R extends ConnectRecord<R>> implements Transformation<R> {
  ...
  private Field getArrayElementField(Schema schema) {
    ...
    if (arrayField == null) {
      ...
      throw new IllegalArgumentException("Root level field '" + arrayFieldName + "' is not a struct");
    }
    ...
    if (innerElementField == null) {
      throw new IllegalArgumentException("Missing array element field: '" + elementWrapperFieldName + "'");
    } else if (innerElementField.schema().type() != Schema.Type.STRUCT) {
      throw new IllegalArgumentException("Array element wrapper: '" + elementWrapperFieldName + "' is not a struct");
    }
